...
function KartaSkupiny({ nazev, data, ted, minule, popisTed, popisMinule, proti }: {
  nazev: string;
  proti: string;
  ...
  const obaly = data.obaly.filter((o) => o.kusy[ted] || o.kusy[minule]);
  return (
    <section className="card p-3.5 sm:p-5" aria-label={nazev}>
      ...
      {obaly.length === 0 ? (
        <p className="mt-3 text-sm font-semibold text-neutral-500">Ani teď, ani minule se nestáčelo.</p>
      ) : (
        <table className="mt-3 w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-200 text-udaj font-black uppercase tracking-wider text-neutral-500">
              ...
              <th scope="col" className="text-right py-1.5">{popisTed}</th>
              <th scope="col" className="text-right py-1.5">{popisMinule}</th>
            </tr>
          </thead>
          <tbody>
            {obaly.map((o) => (
              <tr key={o.id} className="border-b border-neutral-100 last:border-0">
                ...
                <td className="py-2 text-right tabular-nums font-black text-neutral-900">{o.kusy[ted] ? `${ks(o.kusy[ted])} ks` : '–'}</td>
                <td className="py-2 text-right tabular-nums font-semibold text-neutral-500">{o.kusy[minule] ? `${ks(o.kusy[minule])} ks` : '–'}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <KartaSkupiny nazev="Sudy (KEG)" data={data.sudy} ted={ted} minule={minule} popisTed={popisTed} popisMinule={popisMinule} proti={PROTI[druh]} />
        <KartaSkupiny nazev="Lahve a PET" data={data.lahve} ted={ted} minule={minule} popisTed={popisTed} popisMinule={popisMinule} proti={PROTI[druh]} />
      </div>
