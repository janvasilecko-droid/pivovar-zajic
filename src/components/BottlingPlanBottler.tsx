    ...
    >
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
              {lines.map((l, i) => (
                <span key={i} className="text-udaj font-bold bg-white border border-neutral-200 rounded px-1.5 py-0.5 text-neutral-700 whitespace-nowrap">
                  {l.label} × {l.qty}
                </span>
              ))}
        ...
        <div className="flex items-center gap-1.5 flex-wrap shrink-0">
          {isToday && <span className="text-udaj font-black px-2 py-0.5 rounded-full bg-amber-500 text-neutral-950">DNES</span>}
          {isLate && <span className="text-udaj font-black px-2 py-0.5 rounded-full bg-rose-600 text-white">POZDĚ</span>}
          {plan.planned_date && (
            <span className="text-udaj font-bold text-neutral-600 bg-neutral-100 rounded px-2 py-1 whitespace-nowrap">
      ...
      {plan.note && <p className="text-udaj text-neutral-600 mt-1.5 leading-snug"><MessageCircle className="ikona-text" /> {plan.note}</p>}

      <div className="flex items-center gap-2 mt-2.5 flex-wrap">
        <span className={`text-udaj font-black px-2 py-1 rounded border ${STATUS_CHIP[plan.status] || ''}`}>
        ...
        {plan.status === 'planned' && (
          <>
            <button
              type="button"
              onClick={() => onFill(plan)}
              className="px-3 py-1.5 rounded bg-emerald-700 hover:bg-emerald-800 text-white text-udaj font-black transition shadow-sm tap"
            >
            ...
            <button
              type="button"
              onClick={handleDone}
              className="px-3 py-1.5 rounded bg-amber-500 hover:bg-amber-400 text-neutral-950 text-udaj font-black transition shadow-sm tap"
            >
        ...
        {plan.status === 'done' && isManager && (
          <button
            ...
            className="px-3 py-1.5 rounded bg-neutral-200 hover:bg-neutral-300 text-neutral-700 text-udaj font-black transition tap"
          >
    ...
    <div className="card p-3 mb-5 border-2 border-amber-300/80 bg-white">
      <div className="flex items-center justify-between gap-2 mb-2.5 flex-wrap">
        <span className="font-display font-black text-amber-950 text-xs flex items-center gap-1.5">
          ...
          {groups.today.length > 0 && (
            <span className="text-udaj font-black px-1.5 py-0.5 rounded-full bg-amber-500 text-neutral-950 animate-pulse">DNES: {groups.today.length}</span>
          )}
        </span>
        <span className="text-udaj font-bold text-amber-900/60">
      ...
      {groups.late.length > 0 && (
        <div className="space-y-2 mb-3">
          <div className="text-udaj font-black text-rose-700 uppercase tracking-wide"><AlertTriangle className="ikona-text" /> Zpožděné úkoly</div>
          {groups.late.map((p) => (
      ...
      {groups.today.length > 0 && (
        <div className="space-y-2 mb-3">
          <div className="text-udaj font-black text-amber-700 uppercase tracking-wide"><Calendar className="ikona-text" /> Na dnes</div>
          {groups.today.map((p) => (
      ...
      {groups.upcoming.length > 0 && (
        <div className="space-y-2">
          <div className="text-udaj font-black text-neutral-500 uppercase tracking-wide"><CalendarDays className="ikona-text" /> Připravované</div>
